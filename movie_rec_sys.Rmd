...
docterm_plot_sparse <- docterm_plot %>% removeSparseTerms(0.999)

findFreqTerms(docterm_plot_sparse, lowfreq = 500) %>% sort()

tfidf_matrix <- docterm_plot_sparse %>% as.matrix()

tfidf_matrix[1:5, 1:5]

t <- tfidf_matrix[which(row.names(tfidf_matrix) == 'Memento'),]


t1 <- proxy::dist(tfidf_matrix, t, method = "cosine")
...
cosine_dist_mat <- cosineDist(t, tfidf_matrix)

term_dist <- data.frame(title = row.names(tfidf_matrix), distance = cosine_dist_mat) 
...
docterm_tf <- corpus_plot %>% 
  # DocumentTermMatrix(control = list(weighting = function(x) weightTf(x))) 
  DocumentTermMatrix(control = list(weighting = function(x) weightTf(x)))

lda <- LDA(docterm_tf, k = 10, 
